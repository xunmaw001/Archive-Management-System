	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="保养说明" prop="baoyangshuoming">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="保养说明"
					  v-model="ruleForm.baoyangshuoming" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' v-else-if="ruleForm.baoyangshuoming" label="保养说明" prop="baoyangshuoming">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.baoyangshuoming}}</span>
				</el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.baoyangriqi = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='baoyangshuoming'){
							this.ruleForm.baoyangshuoming = obj[o];
							this.ro.baoyangshuoming = true;
							continue;
						}
						if(o=='baoyangriqi'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shebeibaoyang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shebeibaoyangCrossAddOrUpdateFlag = false;
						 this.parent.search();
